...
const commands = [
  new SlashCommandBuilder()
    ...
    .addStringOption((opt) =>
      opt.setName('emoji').setDescription("L'emoji/badge à afficher (ex: 🛠️ ou [HELPER])").setRequired(true)
    )
    .addStringOption((opt) =>
      opt
        .setName('position')
  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (interaction.commandName === 'role-panel') {
      ...
      const options = await Promise.all(
        selfAssignable.map(async ([roleId, cfg]) => {
          const role = await interaction.guild.roles.fetch(roleId);
          return {
            label: role ? role.name : 'Rôle inconnu',
            value: roleId,
            description: `Ajoute ${cfg.badge} à ton pseudo`,
            emoji: /^\p{Emoji}/u.test(cfg.badge) ? cfg.badge : undefined,
          };
        })
      );
