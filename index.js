  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (interaction.commandName === 'badge-set') {
      ...
      try {
        ...
        // Le badge est bien enregistré (déjà confirmé ci-dessus) — seule la
        // resynchro immédiate des pseudos a échoué. On log sans renvoyer
        // d'erreur à l'utilisateur, sinon un succès aurait l'air d'un échec.
        console.error('⚠️ Échec resynchro pseudos (badge-set) :', err);
      }
    }
    ...
    if (interaction.commandName === 'badge-remove') {
      ...
      try {
        ...
        console.error('⚠️ Échec resynchro pseudos (badge-remove) :', err);
      }
    }
